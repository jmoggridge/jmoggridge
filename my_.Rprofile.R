...
setHook("rstudio.sessionInit", function(newSession) {
  ...
  url <-
    "http://api.weatherapi.com/v1/current.json?key=40a5bdff38304d4885662200212012&q=k1r5r2&aqi=yes"
  ...
  conditon_emoji <- function(c) {
    ...
    set_emoji <- dplyr::case_when(
      c %in% "Sunny" ~ em('sunny'),
      c %in% "Clear" ~ em("crescent_moon"),
      c %in% w$cloud ~ em('cloud'),
      c %in% w$rain1 ~ em('umbrella'),
      c %in% w$sun_behind_small_cloud ~ em('sun_behind_small_cloud'),
      c %in% w$sun_behind_rain_cloud ~ em('sun_behind_rain_cloud'),
      c %in% w$cloud_with_lightning_and_rain ~ em('cloud_with_lightning_and_rain'),
      c %in% w$cloud_with_snow ~ em('cloud_with_snow'),
      c %in% w$snow2 ~ paste0(rep(em('snowman'), 3), collapse = ''),
      c %in% w$ice_pellets ~  paste0(em('snowflake'), em('basketball'), collapse =
                                       ''),
      TRUE ~ em("sunrise_over_mountains")
    )
